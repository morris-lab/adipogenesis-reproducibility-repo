...
https://figshare.com/s/127defff99179c267842

```{r}
merge1 <- readRDS("~/Downloads/23_11_08_p21_refs_merged_integrated.rds")
```
...
```{r}
# own cell is counted as 1 of 20 nearest neighbors with distance 0; find nearest 21 to exclude self
merge1 <- FindNeighbors(merge1, reduction = "integrated.cca", dims = 1:30, k.param = 21, return.neighbor = TRUE)
```

```{r}
mergenn <- merge1@neighbors$RNA.nn@nn.idx[, 2:21]
merge1$dataset[mergenn[1, ]]
```

```{r}
mergenndf <- apply(mergenn, 1, function(x) {merge1$dataset[x]})
mergenndf_p21 <- mergenndf[, which(merge1$dataset == "P21")]
```

```{r}
p21nn_p21 <- apply(mergenndf_p21, 2, function(x) {length(which(x == "P21"))})
p21nn_buechler <- apply(mergenndf_p21, 2, function(x) {length(which(x == "Buechler"))})
p21nn_merrick <- apply(mergenndf_p21, 2, function(x) {length(which(x == "Merrick"))})
p21nn_burl <- apply(mergenndf_p21, 2, function(x) {length(which(x == "Burl"))})
p21nn_phan <- apply(mergenndf_p21, 2, function(x) {length(which(x == "Phan"))})
p21nn_jacob <- apply(mergenndf_p21, 2, function(x) {length(which(x == "Jacob"))})
```

```{r}
nn_origin <- data.frame(freq = c(p21nn_p21, p21nn_merrick, p21nn_buechler, p21nn_burl, p21nn_jacob, p21nn_phan), 
                        dataset = c(rep("P21", length(p21nn_p21)), rep("Merrick", length(p21nn_p21)), rep("Buechler", length(p21nn_p21)), 
                                    rep("Burl", length(p21nn_p21)), rep("Jacob", length(p21nn_p21)), rep("Phan", length(p21nn_p21))))

nn_origin$dataset <- factor(nn_origin$dataset, levels = c("P21", "Merrick", "Jacob", "Burl", "Buechler", "Phan"))
```

```{r}
p1d <- ggplot(nn_origin, aes(x = dataset, y = freq / 20, fill = dataset)) + 
  geom_violin(scale = "width") + theme_classic() + 
  theme(legend.position = "none") +
  scale_fill_manual(values = c("white", "#756BB1", "#E6550D", "#DE2D26", "#3182BD", "#31A354")) +
  stat_summary(fun = "mean", size = 0.3, colour = "red") +
  ylab("Proportion of P21 20 nearest neighbors")
p1d
```
...
```{r}
actual.multi <- read.csv("~/Documents/GitHub/p21_merrick_capybara_transition_scores.csv")
```

```{r}
actual.multi$variable <- gsub("frxn_cell.type_", "", actual.multi$variable)
```

```{r}
s1h <- ggplot(actual.multi, aes(x = variable, y = qp.score, fill = variable)) + geom_boxplot() + theme_classic() + scale_fill_manual(values = colpal12[c(1:8, 10:11)]) + theme(legend.position = "none")
s1h
```
...
```{r}
t.test(p21@assays$RNA@data["F3", which(p21$new_classification == "Interstitial progenitor")],
       p21@assays$RNA@data["F3", which(p21$new_classification == "Transitioning interstitial progenitor")])
```

```{r}
t.test(p21@assays$RNA@data["F3", which(p21$new_classification == "Immature preadipocyte")],
       p21@assays$RNA@data["F3", which(p21$new_classification == "Transitioning interstitial progenitor")])
```

```{r}
t.test(p21@assays$RNA@data["F3", which(p21$new_classification == "Immature preadipocyte")],
       p21@assays$RNA@data["F3", which(p21$new_classification == "Committed preadipocyte")])
```
